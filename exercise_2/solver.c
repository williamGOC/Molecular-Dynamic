...
void main(){
    ...
    int NEQ = 1000;
    int NRUM = 1000;
   class_system *sys = builder(0.005, f);
    
    srand(time(NULL));
    FILE *feop = fopen("Energy.txt","w");
...
/*
Cálculo de la presión instantanea del sistema de partículas.
Keyword Arguments:
sys: class_system * (sistema de partículas).
*/
double pressure(class_system *sys){
    double kinetic_aport = 0;
    double pottial_aport = 0;
    double L = pow(N / RHO, 1.0 / 3.0);
    double aux, dij;
    int i, j;

    for(i = 0; i <N; i ++)
        kinetic_aport += square(sys -> momentum[i][0])+ square(sys -> momentum[i][1]) 
                        + square(sys -> momentum[i][2]);
    ...
    for(i = 0; i < N; i++){
        for (j = i + 1; j < N; j++){
            ...
            if(square(dij) < square(RCUT)){
                aux = pow(dij,-6);
                pottial_aport += 24 * aux * (2 * aux - 1);
            }
        }
    }

    return (kinetic_aport + pottial_aport) / (3 * pow(L, 3));
}


/*
Termalización del sistema. Renormaliza las velocidades según la temperatura
deseada.
Keyword Arguments:
sys: class_system * (sistema de partículas).
*/
void thermals(class_system *sys){
    ...
    for(i = 0; i < N; i++){
        for(j = 0; j < 3; j++)
            aux += square(sys -> momentum[i][j]);
    }

    for(i = 0; i < N; i++){
        for(j = 0; j < 3; j++)
            sys -> momentum[i][j] = sqrt((4 * N * T_0) / aux) * sys -> momentum[i][j];
    }
}
